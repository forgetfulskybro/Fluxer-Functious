export default async (client, message, userId, emojiId, event = 'add') => {
	...
	if (event === 'add') {
		...
		if (db2.dm) {
			const dmContent = error ? `**[${guild.name}]** ${client.translate.get(db2.language, 'Events.messageReactionAdd.noPerms', { role: `**${role.name}**` })}!` : `**[${guild.name}]** ${client.translate.get(db2.language, 'Events.messageReactionAdd.success', { role: `**${role.name}**` })}!`;
	...
	if (db2.dm) {
		const dmContent = error ? `**[${guild.name}]** ${client.translate.get(db2.language, 'Events.messageReactionRemove.noPerms', { role: `**${role.name}**` })}!` : `**[${guild.name}]** ${client.translate.get(db2.language, 'Events.messageReactionRemove.success', { role: `**${role.name}**` })}!`;
