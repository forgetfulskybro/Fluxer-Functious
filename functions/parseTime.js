...
function parseTime(input, tz = 'America/New_York') {
	...
	const getPart = (type) => parseInt(currentParts.find((p) => p.type === type)?.value ?? '0', 10);
	...
	const approxDate = new Date(Date.UTC(targetYear, targetMonth, targetDay, targetHour, targetMinute, 0, 0));
	...
	const messageText = input.slice(0, start) + timestamp + input.slice(end);
