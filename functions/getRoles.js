...
async function getRoles(roles, message, client, db, format = true, position = true) {
	...
	let newRoles = processedRoles.map((processed) => {
		return [...message.guild.roles].map((r) => r).find((role) => processed.toLowerCase() === role[1]?.name?.toLowerCase());
	});
	...
	if (position) {
		...
		if (!botRole) {
			message.reply({ embeds: [new EmbedBuilder().setColor('#FF0000').setDescription(client.translate.get(db.language, 'Events.messageCreate.noBotRole'))] }).catch(() => {
				return;
			});
			return message.react(client.config.emojis.cross).catch(() => {
				return;
		...
		if (positions.length > 0) {
			message
				.reply({ embeds: [new EmbedBuilder().setColor('#FF0000').setDescription(`${client.translate.get(db.language, 'Events.messageCreate.positions')}\n${positions.map((e) => `\`${format ? `{role:${e[1].name}}` : e[1].name}\``)}\n\n${client.translate.get(db.language, 'Events.messageCreate.fix')}`)] })
				.catch(() => {
					return;
				});
			return message.react(client.config.emojis.cross).catch(() => {
