		...
		try {
			...
			const handleRaw = (data) => {
				try {
					...
					if (payload.t === GatewayDispatchEvents.GuildCreate || payload.t === 'GUILD_CREATE') {
						const d = payload.d;
						const gId = d?.id;
						const voiceStates = d?.voice_states;

						if (gId && !processedGuilds.has(gId) && Array.isArray(voiceStates)) {
							...
							for (const state of voiceStates) {
								...
								if (userId && channelId) {
									processVoiceState(gId, userId, channelId, state.member?.user?.bot ?? false);
									seeded++;
								}
							}
							totalSeeded += seeded;
