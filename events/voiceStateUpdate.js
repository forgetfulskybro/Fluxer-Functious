...
export default async (client, oldState, newState) => {
	...
	if (newChannelId && tempChannels.some((c) => c.channelId === newChannelId)) {
		...
		if (wasTemp) {
			...
			if ([...client.observedVoiceUsers.values()].filter((data) => data?.channelId === observe.channelId).length === 0) {
				const tempChan = await guild.channels.get(observe.channelId);
				if (tempChan) await tempChan.delete();

				const updatedTemps = tempChannels.filter((c) => c.channelId !== observe.channelId);
	...
	if (parentChannelId && childChannel && newChannelId === childChannel && channel?.parentId === parentChannelId) {
		...
		if (!client.observedVoiceUsers.get(userId)) {
			...
			await client.database.updateGuild(guildId, {
				tempChannels: [...tempChannels, { ownerId: userId, channelId: voiceChannel.id, parentChannel }],
			});
		}
	}
};
