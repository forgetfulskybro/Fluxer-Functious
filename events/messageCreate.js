...
export default async (client, message) => {
	if (!message?.channel || message.channel.type === 1 || !message.content || message.author.bot) return;

	const me = message.guild ? (message.guild.members.me ?? (await message.guild.members.fetchMe().catch(() => null))) : null;
	...
	if (client.messageCollector.has(message.author.id) && client.messageCollector.get(message.author.id).channelId === message.channelId && !client.messageCollector.get(message.author.id).messageId) return await Collector(client, message, db);

	if (client.messageEdit.has(message.author.id) && client.messageEdit.get(message.author.id).channelId === message.channelId && !client.messageEdit.get(message.author.id).messageId) return await EditCollector(client, message, db);
	...
	if (!member) member = await message.guild.fetchMember(message.author.id).catch(() => null);
	...
	if (!commandfile.config.available && commandfile.config.available !== 'Owner' && !client.config.owners.includes(message.author.id)) {
		return message
			.reply(
				{
					embeds: [new EmbedBuilder().setColor('#FF0000').setDescription(client.translate.get(db.language, 'Events.messageCreate.unavail'))],
				},
				false,
			)
			.catch(() => {});
	...
	if (commandfile.config.permissions?.name && !member?.permissions.has(commandfile.config.permissions.bitField) && !client.config.owners.includes(message.author.id) && !bypass) {
		return message
			.reply({
				embeds: [new EmbedBuilder().setColor('#FF0000').setDescription(`${client.translate.get(db.language, 'Events.messageCreate.perms')}.\n${client.translate.get(db.language, 'Events.messageCreate.perms2')}: [${String(commandfile.config.permissions.name)}]`)],
			})
			.catch(() => {});
	}
	...
	if (used) {
		...
		const embed = new EmbedBuilder().setColor('#A52F05').setDescription(`<@${message.author.id}>, ${client.translate.get(db.language, 'Events.messageCreate.wait', { time: `\`${uremaining}\``, cmd: `\`${cmd}\`` })}`);
