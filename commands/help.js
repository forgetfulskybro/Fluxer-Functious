...
export async function run(client, message, args, db) {
	if ((args[0] && client.commands.filter((c) => c.config.available !== 'Owner').get(args[0].toLowerCase())) || client.commands.filter((c) => c.config.available !== 'Owner').find((c) => c.config.aliases.includes(args[0].toLowerCase()))) {
		...
		const embed = new EmbedBuilder()
			...
			.setDescription(client.translate.get(db.language, `Commands.${command.config.name}.description`))
			.addFields(
				{
					name: client.translate.get(db.language, 'Commands.help.embeds.first.cmdAvail'),
					value: command.config.available ? client.translate.get(db.language, 'Commands.help.embeds.first.cmdAvail2') : client.translate.get(db.language, 'Commands.help.embeds.first.cmdAvail3'),
					inline: true,
				},
				{
				...
				},
				{
					...
					value: command.config.permissions?.name ? command.config.permissions.name : client.translate.get(db.language, 'Commands.help.embeds.first.cmdPerms2'),
					inline: true,
				},
			);
	...
	const embed = new EmbedBuilder()
		.setDescription(
			`${client.translate.get(db.language, 'Commands.help.embeds.second.start')}\n${client.commands
				.filter((c) => c.config.available && c.config.available !== 'Owner')
				.map((c) => `\`${c.config.name}\``)
				.join(', ')}${
				client.commands.filter((c) => c.config.available === false).size > 0
					? `\n\n${client.translate.get(db.language, 'Commands.help.embeds.second.middle')}\n${client.commands
							.filter((c) => c.config.available === false)
							.map((c) => `\`${c.config.name}\``)
							.join(', ')}`
					: ''
