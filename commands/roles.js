...
import Paginator from '../functions/pagination';
	...
	switch (args[0]?.toLowerCase()) {
		...
		case 'edit':
			...
			const edittimeout = setTimeout(async () => {
				...
				await (await client.channels.resolve(client.messageEdit.get(editcoll.user).channelId))?.messages
					?.fetch(client.messageEdit.get(editcoll.user).messageId)
					?.edit({ embeds: [ended] })
					.catch(() => {});
  ...
  **This is an example message**
  ...
  `,
		...
		case 'create':
			...
			if (picked.filter((e) => e).length === 2)
				return message.reply({
					embeds: [
						new EmbedBuilder()
							.setDescription(`${client.translate.get(db.language, 'Commands.roles.pick')}\n\n${client.translate.get(db.language, 'Commands.language.example')}: ${db.prefix}roles content <#${channel.id}>\n${client.translate.get(db.language, 'Commands.language.example')}: ${db.prefix}roles embed <#${channel.id}>`)
							.setColor(`#FF0000`),
					],
				});
