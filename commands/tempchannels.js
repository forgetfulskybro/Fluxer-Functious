...
export async function run(client, message, args, db) {
	...
	async function resolveCategory(guild, categoryInput, client, db, command) {
		...
		if (categoryInput === '') {
			return {
				valid: false,
				embed: createEmbed(EMBED_COLORS.ERROR, null, `${client.translate.get(db.language, 'Commands.tempchannels.customCat')}\n**${client.translate.get(db.language, 'Commands.tempchannels.example')}**:\n\`${db.prefix}tc ${command} {category:Temp Channels}\``),
			};
		}

		await guild.fetchChannels();
		const byId = guild?.channels?.resolve?.(categoryInput) ?? null;
		const byName = guild?.channels?.find?.((c) => c?.name?.toLowerCase?.() === categoryInput.toLowerCase()) ?? null;
	...
	function checkSetupExists(db, client, message) {
		if (!db.parentChannel || !db.childChannel) {
			message.reply({
				embeds: [createEmbed(EMBED_COLORS.ERROR, null, `${client.translate.get(db.language, 'Commands.tempchannels.notSetup')}`)],
			});
			return false;
		}
		return true;
	...
	function checkNotSetupForSet(db, client, message, baseCommand, reset = false) {
		if ((db.parentChannel || db.childChannel) && !reset) {
			...
			message.reply({
				embeds: [createEmbed(EMBED_COLORS.ERROR, null, `${client.translate.get(db.language, 'Commands.tempchannels.alreadySetup', { option: '{reset}' })}\n\n\`\`\`\n${suggestion}\n\`\`\``)],
			});
			return false;
		}
		return true;
				...
				case 'config':
					{
						...
						if (reset) {
							...
							if (!channelName && !channelLimit && !counting && !targetCategoryId) {
								if (!didReset) {
									return message.reply({
										embeds: [createEmbed(EMBED_COLORS.ERROR, null, client.translate.get(db.language, 'Commands.tempchannels.noReset'))],
									});
								}
								return message.reply({
									embeds: [createEmbed(EMBED_COLORS.SUCCESS, null, client.translate.get(db.language, 'Commands.tempchannels.successReset'))],
								});
							}
						}
						...
						if (targetCategoryId) {
							...
							if (!category) {
								return message.reply({
									embeds: [createEmbed(EMBED_COLORS.ERROR, null, client.translate.get(db.language, 'Commands.tempchannels.failedSetup'))],
								});
							}

							voiceChannel = await message.guild
								.createChannel({
									type: 2,
									name: client.translate.get(db.language, 'Commands.tempchannels.joinCreate'),
									parent_id: category.id,
		...
		case 'edit':
			{
				...
				message.reply({
					embeds: [createEmbed(EMBED_COLORS.SUCCESS, null, `${client.translate.get(db.language, 'Commands.tempchannels.successEdit')}:\n\n${changedFields}`)],
				});
			}
			break;
		...
		case 'delete':
			{
				...
				if (fieldsToDelete.length === 0) {
					return message.reply({
						embeds: [createEmbed(EMBED_COLORS.ERROR, null, `${client.translate.get(db.language, 'Commands.tempchannels.delete')}.\n**${client.translate.get(db.language, 'Commands.tempchannels.example')}**:\n\`${db.prefix}tc delete {counting} {limit} {name} {category}\``)],
					});
				}
				...
				if (invalidFields.length > 0) {
					return message.reply({
						embeds: [createEmbed(EMBED_COLORS.ERROR, null, `${client.translate.get(db.language, 'Commands.tempchannels.invalid')}: ${invalidFields.join(', ')}\n${client.translate.get(db.language, 'Commands.tempchannels.valid')}: ${VALID_FIELDS.join(', ')}`)],
					});
				}
				...
				const deletedFields = fieldsToDelete
					.map((field) => {
				...
				message.reply({
					embeds: [createEmbed(EMBED_COLORS.SUCCESS, null, `${client.translate.get(db.language, 'Commands.tempchannels.successDelete')}: ${deletedFields}`)],
				});
			}
			break;
	}
