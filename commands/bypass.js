...
export async function run(client, message, args, db) {
	const permCmds = client.commands.filter((c) => c.config.permissions?.bitField).map((c) => c.config.name);
	permCmds.push('timezone');
	permCmds.push('all');

	const arg = args
		.join(' ')
		.replace(/add|remove|edit/gi, '')
		.split('|')
		.map((x) => x.trim())
		.filter((x) => x);
	const role = arg[0];
	...
	if (args[0]) {
		if (!role && args[0].toLowerCase() !== 'view')
			return message.reply({
				embeds: [new EmbedBuilder().setDescription(`${client.translate.get(db.language, 'Commands.bypass.roleName')}:\n\`${db.prefix}bypass ${args[0].toLowerCase()} Moderator ${args[0].toLowerCase() !== 'remove' ? `| roles, giveaways` : ``}\``).setColor(`#FF0000`)],
			});

		if (!cmds && args[0]?.toLowerCase() !== 'view' && args[0]?.toLowerCase() !== 'remove')
			return message.reply({
				embeds: [
					new EmbedBuilder()
	...
	switch (args[0]?.toLowerCase()) {
		...
		case 'add':
			if (db.bypassRoles.length > 15)
				return message.reply({
					embeds: [new EmbedBuilder().setDescription(client.translate.get(db.language, 'Commands.bypass.maximumBypass')).setColor(`#FF0000`)],
				});

			if (db.bypassRoles.length > 0 && db.bypassRoles.find((r) => r.role === roleIds[0][0]))
				return message.reply({
					embeds: [
						new EmbedBuilder()
							.setDescription(
								`${client.translate.get(db.language, 'Commands.bypass.roleAlready')}: \`${db.bypassRoles.find((r) => r.role === roleIds[0][0])?.commands.join(', ')}\`\n\n${client.translate.get(db.language, 'Commands.bypass.roleUse')}\n\`${db.prefix}bypass edit ${role} | [${client.translate.get(db.language, 'Commands.bypass.lockCommands')}, e.g. giveaway, roles]\``,
							)
							.setColor(`#FF0000`),
					],
				});
			...
			if (invalidCmds.length > 0)
				return message.reply({
					embeds: [new EmbedBuilder().setDescription(`${client.translate.get(db.language, 'Commands.bypass.invalidCmds')}\n**${client.translate.get(db.language, 'Commands.bypass.commands')}**: \`${invalidCmds.join(', ')}\``).setColor(`#FF0000`)],
				});

			if (cmds.includes('all')) {
				message.reply({
					embeds: [new EmbedBuilder().setDescription(`${client.translate.get(db.language, 'Commands.bypass.successAll')} <@&${roleIds[0][0]}>`).setColor(`#A52F05`)],
				});

				type = ['all'];
			} else {
				message.reply({
					embeds: [new EmbedBuilder().setDescription(`${client.translate.get(db.language, 'Commands.bypass.success', { role: `<@&${roleIds[0][0]}>` })}:\n\`${cmds.join(', ')}\``).setColor(`#A52F05`)],
				});
		...
		case 'remove':
			if (!db.bypassRoles.find((r) => r.role === roleIds[0][0]))
				return message.reply({
					embeds: [new EmbedBuilder().setDescription(client.translate.get(db.language, 'Commands.bypass.notBypassed')).setColor(`#FF0000`)],
				});

			message.reply({
				embeds: [new EmbedBuilder().setDescription(client.translate.get(db.language, 'Commands.bypass.removeSuccess', { role: `<@&${roleIds[0][0]}>` })).setColor(`#A52F05`)],
			});
		...
		case 'edit':
			...
			if (!bypassedRole)
				return message.reply({
					embeds: [new EmbedBuilder().setDescription(client.translate.get(db.language, 'Commands.bypass.notBypassedEdit')).setColor(`#FF0000`)],
				});
		...
		case 'view':
			if (db.bypassRoles.length === 0)
				return message.reply({
					embeds: [new EmbedBuilder().setDescription(client.translate.get(db.language, 'Commands.bypass.noRolesView')).setColor(`#FF0000`)],
				});
