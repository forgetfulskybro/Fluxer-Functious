...
export async function run(client, message, args, db) {
	...
	if (!client.translate.availableLanguages.includes(args[0])) return message.reply(client.translate.get(db.language, 'Commands.language.notAvailable'), { ping: false });

	await client.database.updateGuild(message.guildId, { language: args[0] });
	await message.reply(`${client.translate.get(args[0], 'Commands.language.success')} **${args[0]}**`, { ping: false });
}
