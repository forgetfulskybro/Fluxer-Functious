...
export async function run(client, message, args, db) {
	...
	switch (args[0]) {
		...
		case 'toggle':
			...
			if (!member?.permissions.has(PermissionFlags.ManageGuild) && !client.config.owners.includes(message.author.id) && !bypass)
		...
		case 'set':
			if (!args[1] || !isValid(args[1]) || !dateType(args[1]))
				return message.reply({
					embeds: [
						new EmbedBuilder()
							.setColor('#FF0000')
							.setDescription(
								`${client.translate.get(db.language, 'Commands.timezone.validTime', { timezone: '`America/New_York`' })} [${client.translate.get(db.language, 'Commands.timezone.timeZonePicker')}](https://zones.arilyn.cc/).\n\n${client.translate.get(db.language, 'Commands.timezone.validTime2')}: \`${db.prefix}timezone set America/New_York\``,
							),
					],
				});

			const userTimezone = db.userTimezones.find((u) => u.userId === message.author.id);
			if (userTimezone?.timezone.toLowerCase() === args[1]?.toLowerCase())
				return message.reply({
					embeds: [new EmbedBuilder().setColor('#FF0000').setDescription(`${client.translate.get(db.language, 'Commands.timezone.sameTime')} [${client.translate.get(db.language, 'Commands.timezone.timeZonePicker')}](https://zones.arilyn.cc/)`)],
				});
		...
		case 'remove':
			if (!db.userTimezones.find((u) => u.userId === message.author.id))
				return message.reply({
					embeds: [
						new EmbedBuilder()
							.setColor('#FF0000')
							.setDescription(
								`${client.translate.get(db.language, 'Commands.timezone.noTime', { timezonepicker: `[${client.translate.get(db.language, 'Commands.timezone.timeZonePicker')}](https://zones.arilyn.cc/)` })}:\n\`${db.prefix}timezone set [${client.translate.get(db.language, 'Commands.timezone.timezone')}, e.g. America/New_York]\``,
							),
					],
				});
